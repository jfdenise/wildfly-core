...
public class BatchDiscardCommand implements Command<CLICommandInvocation> {
    ...
    public CommandResult execute(CLICommandInvocation commandInvocation)
        ...
        if (help) {
            commandInvocation.println(commandInvocation.getHelpInfo("batch " + BatchCommand.DISCARD));
            return CommandResult.SUCCESS;
        }
        //Legacy compliance
        CommandResult res = BatchCommand.handle(commandInvocation, BatchCommand.DISCARD);
        if (res != null) {
            return res;
        }
