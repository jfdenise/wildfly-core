...
/**
 * Redirection for command recording.
 *
 * @author jfdenise
 */
class CommandsRecorderControlFlow implements CommandLineRedirection {
    ...
    public void handle(CommandContext ctx) throws CommandLineException {
        ...
        if (line.getFormat() == CommandFormat.INSTANCE) {
            ...
            if (opName.equals("commands")
                    && (line.getOriginalLine().contains("stop-record")
                    || line.getOriginalLine().contains("list-recorded")
                    || line.getOriginalLine().contains("rewind-record"))) {
                registration.handle(line);
    ...
    @Override
    public void handleLineFormatError(String line) {
        recorded.add(line);
    }
    ...
    String rewind() {
        ...
        return recorded.remove(recorded.size() - 1);
    }

    void trash(CLICommandInvocation ctx) throws CommandLineException {
        recorded.clear();
