 ...
 */
@GroupCommandDefinition(name = "holdback", description = "")
public class BatchHoldbackCommand implements Command<CliCommandInvocation> {
    ...
    public CommandResult execute(CliCommandInvocation commandInvocation)
        ...
        if (!batchManager.isBatchActive()) {
            throw new RuntimeException("No active batch to holdback.");
        }

        if (name == null || name.isEmpty()) {
            throw new RuntimeException("No batch name to holdback");
        }
        String batchName = name.get(0);
        if (batchManager.isHeldback(batchName)) {
            throw new RuntimeException("There already is "
                    + (batchName == null ? "unnamed" : "'" + batchName
                            + "'") + " batch held back.");
        }

        if (!batchManager.holdbackActiveBatch(batchName)) {
            throw new RuntimeException("Failed to holdback the batch.");
        }
        return null;
    }
