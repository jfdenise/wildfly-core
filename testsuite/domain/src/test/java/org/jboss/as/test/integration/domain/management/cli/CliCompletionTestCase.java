...
public class CliCompletionTestCase {
    ...
    public void testCommandsCompletion() throws Exception {
        ...
        try {
            ...
            {
                String cmd = "batch load-";
                List<String> candidates = new ArrayList<>();
                ctx.getDefaultCommandCompleter().complete(ctx, cmd,
                        cmd.length(), candidates);
                assertTrue(candidates.toString(), candidates.size() == 1);
                assertTrue(candidates.toString(), candidates.contains("batch load-file"));
            }

//            { // AESH-442 XXX jfdenise, uncomment when fixed.
    ...
    public void testCommandsCompletion2() throws Exception {
        ...
        try {
...

            {
                ...
                ctx.getDefaultCommandCompleter().complete(ctx, cmd,
                        cmd.length(), candidates);
                assertTrue(candidates.toString(), candidates.contains("deployment deploy-url"));
                assertTrue(candidates.toString(), candidates.contains("deployment deploy-file"));
            }
            ...
            {
                String cmd = "deploy enable ccc ";
                List<String> candidates = new ArrayList<>();
                ctx.getDefaultCommandCompleter().complete(ctx, cmd,
                        cmd.length(), candidates);
                assertFalse(candidates.toString(), candidates.contains("--headers="));
            }
    ...
    public void testCommandsCompletion3() throws Exception {
        ...
        try {
            ...
            }
            {
                ...
                try {
                    String cmd = "batch edit-line --line-number=1 /serv";
                    List<String> candidates = new ArrayList<>();
                    ctx.getDefaultCommandCompleter().complete(ctx, cmd,
                            cmd.length(), candidates);
                    assertTrue(candidates.toString(), candidates.size() == 1);
        ...
        try {
            {
                ...
                ctx.getDefaultCommandCompleter().complete(ctx, cmd,
                        cmd.length(), candidates);
                assertTrue(candidates.toString(), candidates.size() == 2);
                assertEquals(candidates.toString(), Arrays.asList("attachment display",
                        "attachment save"), candidates);
            }
            {
...

            {
                ...
                ctx.getDefaultCommandCompleter().complete(ctx, cmd,
                        cmd.length(), candidates);
                assertTrue(candidates.toString(), candidates.size() == 1);
                assertEquals(candidates.toString(), Arrays.asList("attachment save"),
                        candidates);
            }
            ...
            {
                String cmd = "attachment save --operation=:read-resource() --file=xxx ";
                List<String> candidates = new ArrayList<>();
                ctx.getDefaultCommandCompleter().complete(ctx, cmd,
                        cmd.length(), candidates);
                assertTrue(candidates.toString(), candidates.size() == 1);
                assertEquals(candidates.toString(), Arrays.asList("--overwrite"),
                        candidates);
            }
